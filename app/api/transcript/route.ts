    ...
    try {
      ...
      try {
        ...
        if (innerError?.message?.includes('No transcripts are available')) {
          console.log(`[transcript] Default fetch failed for ${videoId} via ${strategyLabel}. Listing all available languages...`);
          try {
            ...
            if (languages && languages.length > 0) {
              ...
              for (const track of sortedLanguages) {
                try {
                  ...
                  const langLabel = (track as any).languageName || track.language || langCode;
                  ...
                  console.warn(`[transcript] Retry with ${track.languageCode} failed, trying next track...`);
                }
              }
            ...
            console.error('[transcript] listLanguages failed:', (listError as any).message || listError);
